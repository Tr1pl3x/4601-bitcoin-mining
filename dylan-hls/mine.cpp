...
static void sha256_compress_compact(uint32_t state[8], const uint8_t block[64]) {
    uint32_t w[64];
...
// Calculate the SHA-256 state after processing header bytes 0..63.
//
// The Bitcoin nonce occupies bytes 76..79, so bytes 0..63 remain constant
// throughout the nonce search. This compression is therefore performed once.
static void compute_midstate(const uint8_t header[80], uint32_t midstate[8]) {
    uint8_t block[64];
    ...
    sha256_compress_compact(midstate, block);
}

// Complete double SHA-256 beginning from the state after header bytes 0..63.
//
// header_tail points to header bytes 64..79. The final four bytes contain
// the nonce.
static void sha256d_from_midstate(const uint32_t midstate[8], const uint8_t header_tail[16], uint8_t out[32]) {
    uint8_t block[64];
    uint32_t s[8];

    // Restore the saved state for this nonce.
    for (int i = 0; i < 8; i++) {
        s[i] = midstate[i];
    }

    // First SHA-256, second block:
    // header bytes 64..79 followed by padding.
    for (int i = 0; i < 16; i++) {
        block[i] = header_tail[i];
    }
    ...
    // Original message length = 80 bytes = 640 bits.
    block[62] = 0x02;
    block[63] = 0x80;

    sha256_compress(s, block);

    // Second SHA-256 hashes the first digest.
    uint8_t digest1[32];
    ...
    // Second hash input length = 32 bytes = 256 bits.
    block[62] = 0x01;
    block[63] = 0x00;
...
#pragma HLS interface s_axilite port=return
    ...
    *found_nonce = 0;
    ...
    for (uint32_t k = 0; k < nonce_count; k++) {
        ...
        uint8_t h[32];
        // &hdr[64] passes header bytes 64..79.
        sha256d_from_midstate(midstate, &hdr[64], h);
