// Host program for the OPTIMIZED miner kernel on the Kria KV260.
//
// The optimized kernel takes a host-precomputed midstate (8 words) + the fixed
// block-2 tail (header bytes 64..75), not the full header. This host:
...
//
// Kernel: mine(uint32_t midstate[8], uint8_t tail[12], uint8_t target[32],
//              uint32_t start, uint32_t count, uint32_t* found_nonce, int* found)
//   group_ids: midstate=0, tail=1, target=2, found_nonce=5, found=6.
...
int main(int argc, char** argv) {
    ...
    // The host precomputes the midstate + tail, exactly what the kernel expects.
    BlockHeader base = genesis_header();
    uint8_t header[HEADER_BYTES];
    serialize_header(base, header);
    uint32_t midstate[8];
    compute_midstate(header, midstate);
    uint8_t tail[12];
    memcpy(tail, header + 64, 12);   // header bytes 64..75

    uint8_t easy[32]; memset(easy, 0xff, 32); easy[0] = 0x00; easy[1] = 0x00;
    uint8_t zero[32]; memset(zero, 0x00, 32);

    std::cout << "Open device " << device_index << " (optimized)\n";
    auto device = xrt::device(device_index);
    std::cout << "Load xclbin " << binaryFile << "\n";
    auto uuid = device.load_xclbin(binaryFile);
    auto krnl = xrt::kernel(device, uuid, "mine");

    auto ms_bo  = xrt::bo(device, 8 * sizeof(uint32_t), krnl.group_id(0));
    auto tl_bo  = xrt::bo(device, 12,               krnl.group_id(1));
    auto tgt_bo = xrt::bo(device, 32,               krnl.group_id(2));
    auto fn_bo  = xrt::bo(device, sizeof(uint32_t), krnl.group_id(5));
    auto fd_bo  = xrt::bo(device, sizeof(int),      krnl.group_id(6));
    auto ms_map  = ms_bo.map<uint32_t*>();
    auto tl_map  = tl_bo.map<uint8_t*>();
    auto tgt_map = tgt_bo.map<uint8_t*>();
    auto fn_map  = fn_bo.map<uint32_t*>();
    auto fd_map  = fd_bo.map<int*>();

    memcpy(ms_map, midstate, 8 * sizeof(uint32_t));
    memcpy(tl_map, tail, 12);
    ms_bo.sync(XCL_BO_SYNC_BO_TO_DEVICE);
    tl_bo.sync(XCL_BO_SYNC_BO_TO_DEVICE);

    // 1. correctness
    memcpy(tgt_map, easy, 32);
    tgt_bo.sync(XCL_BO_SYNC_BO_TO_DEVICE);
    { auto r = krnl(ms_bo, tl_bo, tgt_bo, (uint32_t)0, (uint32_t)20000, fn_bo, fd_bo);
      r.wait();
    ...
    for (int r = 0; r < N; r++) {
        ...
        auto rn  = krnl(ms_bo, tl_bo, tgt_bo, (uint32_t)0, (uint32_t)bench_count, fn_bo, fd_bo);
        rn.wait();
    ...
    std::cout << "PL HARDWARE (optimized kernel @ 200 MHz)\n";
